    ...
    start(env) {
        ...
        download._download = async function(options) {
            ...
            try {
                ...
                // Parse URL to extract essential parameters only
                ...
                // Keep only essential parameters (remove huge context that causes 414 error)
                const essentialParams = new URLSearchParams();
                if (params.has('options')) {
                    essentialParams.set('options', params.get('options'));
                }
                // Add minimal context if needed
                if (params.has('context')) {
                    try {
                        const contextObj = JSON.parse(decodeURIComponent(params.get('context')));
                        // Keep only essential context fields
                        const minimalContext = {
                            lang: contextObj.lang,
                            tz: contextObj.tz,
                            uid: contextObj.uid,
                            allowed_company_ids: contextObj.allowed_company_ids
                        };
                        essentialParams.set('context', JSON.stringify(minimalContext));
                    } catch (e) {
                        console.log("[Zebra] Could not parse context, skipping");
                    }
                }

                // Build simplified URL
                const simplifiedUrl = window.location.origin + basePath + '?' + essentialParams.toString();
                console.log("[Zebra] Fetching report from (simplified):", simplifiedUrl);
                console.log("[Zebra] Original query length:", queryString.length, "chars");
                console.log("[Zebra] Simplified query length:", essentialParams.toString().length, "chars");

                // Use GET with simplified URL
